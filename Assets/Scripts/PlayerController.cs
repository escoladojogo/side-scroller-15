...
public class PlayerController : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        if (climb == true)
        {
            playerRigidBody2D.velocity = new Vector2(horizontalMove * runBoost, 5);
        }
        else
        {
            playerRigidBody2D.velocity = new Vector2(horizontalMove * runBoost, playerRigidBody2D.velocity.y);
        }

        if (jump == true)
        {
            playerRigidBody2D.AddForce(new Vector2(0, jumpBoost));
            jump = false;
        }
    }
